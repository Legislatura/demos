...
function LawForm(law, winners) {
  if (!(this instanceof LawForm)) {
    return new LawForm(law, winners);
  };
...
LawForm.prototype.postserialize = function(data) {
  ...
  data.phaseTwo = data.phaseTwo || false;
  data.winner = data.winner || false;
  data.author = data.lawAuthor;
  data.intro = !!data.intro ? data.intro : null;
  data.phaseOne = !!data.phaseOne ? data.phaseOne : null;
