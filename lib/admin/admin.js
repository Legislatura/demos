...
page('/admin/laws/create', laws.middleware, tags.middleware, winners.middleware, function (ctx, next) {
  var content = o('.admin-content');

  // render new law form
  var form = new LawForm(null, winners.items);
...
page('/admin/laws/:id', tags.middleware, laws.middleware, winners.middleware, loadLaw, function (ctx, next) {
  // force section for edit
  // as part of list
  sidebar.set('laws');

  // render law form for edition
  var form = new LawForm(ctx.law, winners.items);
  form.replace('.admin-content');
