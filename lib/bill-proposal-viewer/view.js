...
BillProposalViewer.prototype.reload = function() {
  ...
  if (citizen.logged()) {
    bills = sortForCitizen(bills, citizen.id, this.lawId);

    if (this.lawId) {
      selected = array(bills).find({ id: this.lawId});
    } else {
      selected = notScoredByCitizen(bills, citizen.id, this.lawId)[0];
    }
  }
...
function sortForCitizen (bills, citizenId, lawId) {

  return scoredByCitizen(bills, citizenId, lawId).concat(notScoredByCitizen(bills, citizenId, lawId));
}

function scoredByCitizen(bills, citizenId, lawId) {
  var out = array(bills).select(function (l) {
    return ~scorers(l).indexOf(citizenId) != 0;
  }).toArray();

  if (array(out).find({ id: lawId })) {
    out = putLast(out, lawId);
  }

  return out;
}

function notScoredByCitizen(bills, citizenId, lawId) {
  var out = array(bills).select(function (l) {
    return ~scorers(l).indexOf(citizenId) == 0;
  }).toArray();

  if (array(out).find({ id: lawId })) {
    out = putFirst(out, lawId);
  }
