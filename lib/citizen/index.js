...
app.get('/citizen/me', restrict, function (req, res) {
  ...
  res.json(expose('id firstName lastName email gravatar() staff profilePictureUrl sawTutorial notifications')(req.user));
});

app.post('/citizen/tutorial', restrict, function(req, res) {
  log('Request /citizen/tutorial');
  api.citizen.setTutorialStatus(req.user.id, true, function(err) {
...
app.get('/citizen/:id', restrict, function (req, res) {
  ...
  api.citizen.get(req.params.id, function (err, citizen) {
    ...
    res.json(expose('id fullName gravatar() profilePictureUrl notifications sawTutorial')(citizen));
  });
});
