...
/**
 * Set new value for `sawTutorial` column
 *
 * @param {String|ObjectId} id Citizen's `id`
 * @param {Boolean} newStatus new value
 @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 * @api public
 */

exports.setTutorialStatus = function setTutorialStatus(id, newStatus, fn) {
  log('Trying to modify tutorial status for Citizen %s', id);
  exports.get(id, function(err, citizen) {
    citizen.set({ sawTutorial: newStatus });
    citizen.save(function(err, citizen){
      if (!err) return log('Saved citizen %s', citizen.id), fn(null, citizen);
