...
exports.all = function all(fn) {
  ...
  var fields = 'id lawId mediaTitle summary source tag participants votes createdAt updatedAt closingAt ' +
    ...
    'party partyUrl backgroundColor fontColor picture scores scoreParticipants nothings littles somewhats verys phaseTwo intro';
...
exports.winners = function (fn) {
  ...
  Law
  .find({ phaseTwo: false, winner: true, deletedAt: null, public: true })
  .select(fields)
  .exec(function (err, laws) {
    if (err) {
      log('Found error %j', err);
...
exports.losers = function (fn) {
  ...
  Law
  .find({ phaseTwo: false, winner: false, deletedAt: null, public: true })
  .select(fields)
  .exec(function (err, laws) {
    if (err) {
      log('Found error %j', err);
