...
exports.all = function all(fn) {
  ...
  // var fields = 'id lawId mediaTitle summary source tag participants votes createdAt updatedAt closingAt ' +
  //   'publishedAt deletedAt status open closed links author authorUrl authorUrl authorPictureUrl ' +
  //   'party partyUrl backgroundColor fontColor picture scores scoreParticipants nothings littles somewhats verys phaseTwo intro staffTitle singular';
  var fields = 'id lawId mediaTitle summary source tag createdAt updatedAt closingAt ' +
    'publishedAt links author authorUrl authorPictureUrl ' +
    'party partyUrl backgroundColor fontColor picture phaseTwo staffTitle';
...
exports.results = function results(fn) {
  log('Looking for laws scoring results.')

  var fields = 'id mediaTitle summary source scores scored author authorUrl authorPictureUrl '
  ...
  Law
  .find({ deletedAt: null, phaseTwo: false })
  .select(fields)
  .sort('-totalScore')
  .exec(function (err, laws) {
    if (err) {
    ...
    log('Delivering law scoring results %j', pluck(laws, 'id'));
    fn(null, laws);
  });
...
/**
<<<<<<< HEAD
 * Get laws that won the citizen scoring
 *
 * @param {Function} fn callback function
 *   - 'err' error found on query or `null`
 *   - 'laws' list items found or `undefined`
...
exports.winners = function (fn) {
  log('Looking for winner laws.')

  var fields = 'id lawId mediaTitle summary source author authorUrl authorUrl authorPictureUrl ' +
    'party partyUrl backgroundColor fontColor picture intro';

  Law
  .find({ phaseTwo: false, winner: true, deletedAt: null })
  .select(fields)
  .sort('-totalScore')
  .exec(function (err, laws) {
    if (err) {
    ...
    log('Delivering winner laws %j', pluck(laws, 'id'));
    fn(null, laws);
  });
...
/**
 * Get laws that lost the citizen scoring
 *
 * @param {Function} fn callback function
 *   - 'err' error found on query or `null`
 *   - 'laws' list items found or `undefined`
...
exports.losers = function (fn) {
  log('Looking for loser laws.')

  var fields = 'id lawId mediaTitle summary source author authorUrl authorUrl authorPictureUrl ' +
    'party partyUrl backgroundColor fontColor picture intro';

  Law
  .find({ phaseTwo: false, winner: false, deletedAt: null })
  .select(fields)
  .exec(function (err, laws) {
    if (err) {
      log('Found error %j', err);
    ...
    log('Delivering loser laws %j', pluck(laws, 'id'));
    fn(null, laws);
  });
...
exports.phaseOne = function (id, fn) {
  log('Looking for laws which phaseOne is %s', id)

  var fields = 'id lawId mediaTitle participants singular order';

  Law
  .find({ phaseOne: id, deletedAt: null })
  .select(fields)
  .exec(function (err, laws) {
    if (err) {
      log('Found error %j', err);
    ...
    log('Delivering laws with phaseOne %s %j', id, pluck(laws, 'id'));
    fn(null, laws);
  });
...
exports.update = function update(id, data, fn) {
  ...
  tagApi.searchOne(data.tag, function (err, tag) {
    ...
    function onget(err, law) {
      ...
      if (data.intro) {
        exports.get(data.intro, function (err, intro) {
          if (err) {
            log('Found error %s', err.message);
            return fn(err);
          };
...
exports.get = function get(id, fn) {
  ...
  var fields = '-scores -scoreParticipants -totalScore -totalNothings -totalLittles -totalSomewhats -totalVerys -scoreCensus';
