...
app.get('/law/all', function (req, res) {
  ...
  api.law.all(function(err, lawDocs) {
    ...
    var keys = [
      'id lawId mediaTitle status summary source open closed public draft deleted',
      ...
      'scores scoreParticipants nothings littles somewhats verys phaseTwo singular'
    ];

    if (req.user && req.user.staff) {
      keys.push('staffTitle');
    }

    keys = keys.join(' ');

    function toJSON(lawDoc) {
      return lawDoc.toJSON();
    }

    res.json(lawDocs.map(toJSON).map(expose(keys)));
  });
});

app.get('/law/results', function (req, res) {
  log('Request /law/results');

  api.law.results(function(err, lawDocs) {
    if (err) return _handleError(err, req, res);

    // add voted flag to returning documents
    lawDocs.forEach(function(law) {
      if (!req.user) return law.scored = false;
      law.scored = law.scoredBy(req.user);
    });

    log('Serving laws %j', pluck(lawDocs, "id"));

    var keys = [
      'id mediaTitle summary source scored author authorUrl authorPictureUrl',
      'party partyUrl backgroundColor fontColor picture',
      'totalNothings totalLittles totalSomewhats totalVerys totalScore scoreCensus'
    ].join(' ');
...
app.get('/law/winners', function (req, res) {
  log('Request /law/winners');

  api.law.winners(function(err, lawDocs) {
    if (err) return _handleError(err, req, res);

    log('Serving winner laws %j', pluck(lawDocs, "id"));

    var keys = [
      'id lawId mediaTitle summary source author authorUrl authorPictureUrl',
      'party partyUrl backgroundColor fontColor picture intro order'
    ].join(' ');
...
app.get('/law/losers', function (req, res) {
  log('Request /law/losers');

  api.law.losers(function(err, lawDocs) {
    if (err) return _handleError(err, req, res);
...
app.get('/law/phase-two/:id', function (req, res) {
  log('Request /law/phase-two/%s', req.params.id);

  api.law.phaseOne(req.params.id, function(err, lawDocs) {
    if (err) return _handleError(err, req, res);

    log('Serving laws with phaseOne %j', pluck(lawDocs, "id"));

    var keys = [
      'id lawId mediaTitle participants singular order'
    ].join(' ');
...
app.get('/law/:id', function (req, res) {
  ...
  api.law.get(req.params.id, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'scores scoreParticipants nothings littles somewhats verys phaseTwo intro winner phaseOne order'
    ];
...
app.get('/law-phase-two/:id', function (req, res) {
  ...
  api.law.get(req.params.id, function (err, lawDoc) {
    ...
    log('Serving law %s', lawDoc.id);
    var keys = [
      'id lawId mediaTitle summary clauses source participants',
      'upvotes downvotes abstentions closingAt votable clauseTruncationText links author',
      'authorUrl authorPictureUrl party partyUrl',
      'phaseOne.id phaseOne.backgroundColor phaseOne.fontColor phaseOne.mediaTitle',
    ];
...
app.post('/law/create', restrict, staff, function (req, res, next) {
  ...
  api.law.create(req.body, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl phaseTwo'
    ].join(' ');
...
app.post('/law/:id', restrict, staff, function (req, res) {
  ...
  api.law.update(req.params.id, req.body, function (err, lawDoc) {
    ...
    var keys = [
      ...
      'status open closed public draft deleted tag participants',
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl phaseTwo order'
    ].join(' ');
