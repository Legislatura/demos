...
LawSchema.options.toJSON.transform = function(doc, ret, options) {
  ...
  if (ret.upvotes) ret.upvotes = unique(ret.upvotes.map(function(v) { return v.author.toString(); }));
  if (ret.downvotes) ret.downvotes = unique(ret.downvotes.map(function(v) { return v.author.toString(); }));
  if (ret.abstentions) ret.abstentions = unique(ret.abstentions.map(function(v) { return v.author.toString(); }));
